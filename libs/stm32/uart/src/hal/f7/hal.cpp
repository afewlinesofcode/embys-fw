// F7 UART HAL implementation (UART V2 peripheral, ISR/RDR/TDR/ICR layout).
// USART1/6 are on APB2; USART2/3/4/5 are on APB1.
// PPRE2 (APB2 prescaler) is RCC_CFGR bits [15:13] on F7.
// PPRE1 (APB1 prescaler) is RCC_CFGR bits [12:10] on F7.
...
pclk1_hz()
{
  return SystemCoreClock / ppre_div((RCC->CFGR >> 10U) & 0x7U);
}

static uint32_t
pclk2_hz()
{
  return SystemCoreClock / ppre_div((RCC->CFGR >> 13U) & 0x7U);
}
            ...
            StopBits stop_bits, Parity parity)
{
  ...
  {
    SET_BIT_V(RCC->APB1ENR, RCC_APB1ENR_UART4EN);
    SET_BIT_V(RCC->APB1RSTR, RCC_APB1RSTR_UART4RST);
    CLEAR_BIT_V(RCC->APB1RSTR, RCC_APB1RSTR_UART4RST);
    pclk = pclk1_hz();
  }
  else if (usart == UART5)
  {
    SET_BIT_V(RCC->APB1ENR, RCC_APB1ENR_UART5EN);
    SET_BIT_V(RCC->APB1RSTR, RCC_APB1RSTR_UART5RST);
    CLEAR_BIT_V(RCC->APB1RSTR, RCC_APB1RSTR_UART5RST);
    pclk = pclk1_hz();
  }
...
disable_uart(USART_TypeDef *usart)
{
  ...
  if (usart == USART1)
    ...
    CLEAR_BIT_V(RCC->APB1ENR, RCC_APB1ENR_UART4EN);
  else if (usart == UART5)
    CLEAR_BIT_V(RCC->APB1ENR, RCC_APB1ENR_UART5EN);
  else if (usart == USART6)
