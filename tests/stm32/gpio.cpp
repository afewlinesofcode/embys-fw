...
TEST_CASE_FIXTURE(GpioLoopFixture, "Pin: enable with PULL_DOWN clears ODR bit")
{
  bus.enable();
...
TEST_CASE_FIXTURE(GpioLoopFixture, "Pin: disable resets pin to floating input")
{
  bus.enable();
                  ...
                  "Pin: rising edge triggers callback with value=1")
{
  ...
  pin.set_callback({[](void *ctx, uint8_t v)
                    { static_cast<std::vector<uint8_t> *>(ctx)->push_back(v); },
                    &calls});
