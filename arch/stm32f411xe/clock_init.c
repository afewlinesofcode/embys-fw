...
clock_init_100mhz(void)
{
  ...
  if (hse_ok)
  {
    ...
    RCC->PLLCFGR = (25U) | (200U << RCC_PLLCFGR_PLLN_Pos) |
                   (0U << RCC_PLLCFGR_PLLP_Pos) | RCC_PLLCFGR_PLLSRC_HSE |
                   (4U << RCC_PLLCFGR_PLLQ_Pos);
  }
  else
  {
    // HSI 16 MHz / M=8 → 2 MHz VCO input, ×N=100 → 200 MHz, /P=2 → 100 MHz
    RCC->PLLCFGR = (8U) | (100U << RCC_PLLCFGR_PLLN_Pos) |
                   (0U << RCC_PLLCFGR_PLLP_Pos) | RCC_PLLCFGR_PLLSRC_HSI |
                   (4U << RCC_PLLCFGR_PLLQ_Pos);
  }
